...
export default {
  ...
  async scheduled(event: ScheduledController, env: Env, ctx: ExecutionContext): Promise<void> {
    ...
    switch (event.cron) {
      case CRON_COLLECT:
        console.log(`[cron] ${event.cron} (KST 08:00) — K-Startup 수집`);
        // TODO(수집 담당): runCollect(env)를 여기서 호출한다. 아직 없으므로 no-op.
        break;

      case CRON_DIGEST:
        console.log(`[cron] ${event.cron} (KST 08:30) — D-day 다이제스트`);
        // TODO(디스코드 담당): sendDigest(env)를 여기서 호출한다. 아직 없으므로 no-op.
        break;
